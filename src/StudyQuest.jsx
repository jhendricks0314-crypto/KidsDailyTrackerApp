...
const api = {
  async signup(username, password) {
    const r = await apiRequest("signup", { username, password });
    setToken(r.token);
    return r.parent;
  },
...
function buildMathList(grade, categories) {
  ...
  while (list.length < 10) list.push(blankItem(genMath(grade, cats[list.length % cats.length])));
  return list;
}
...
// Build a full day of questions. Math is always procedural (exact answers).
// Text subjects are generated by the AI using the selected categories, with a
// graceful fallback to the curated generators when offline / on error.
async function buildDay(grade, kid) {
  const out = {};
  out["Math"] = buildMathList(grade, selectedCategoriesFor("Math", kid));

  const textSubjects = SUBJECTS.filter((s) => s.key !== "Math");
  const requests = textSubjects.map((s) => ({
    subject: s.key,
    categories: selectedCategoriesFor(s.key, kid),
    count: 10,
  }));

  let generated = null;
  try {
    generated = await api.generateQuestions(grade, requests);
  } catch {
    generated = null;
  }
...
/* ============================ DEFAULT CHORES ============================ */
const DEFAULT_CHORES = ["Make your bed", "Brush your teeth", "Tidy your room", "Feed the pet"];
...
export default function App() {
  ...
  const loadKidData = useCallback(
    async (kidId) => {
      ...
      if (ch == null) {
        ch = DEFAULT_CHORES.map((t) => ({ id: uid(), title: t }));
...
function AuthScreen({ onAuthed, updateBanner }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const parentObj = mode === "signup" ? await api.signup(username.trim(), password) : await api.login(username.trim(), password);
...
/* ============================ PARENT PANEL ============================ */
function ParentPanel({ parent, kids, refreshKids, activeKid, setActiveKid, date }) {
  ...
  return (
    <div>
      ...
      {section === "password" && <PasswordManager />}
...
function CategoriesManager({ kids, refreshKids, activeKid, setActiveKid }) {
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      await api.updateKid(kid.id, { categories: { selected, custom } });
      await refreshKids();
      setSaved(true);
    } finally {
      setBusy(false);
        ...
        return (
          <div key={s.key} style={{ padding: "14px 0", borderBottom: "1px solid #f0ecf6" }}>
            ...
            {allowCustom ? (
              ...
              <div style={{ fontSize: 12, color: "#9a8fb0", marginTop: 8 }}>
                Math focuses on these operation types so answers stay exact.
              </div>
            )}
          </div>
        );
...
function KidsManager({ kids, refreshKids, activeKid, setActiveKid }) {
  ...
  const commitName = async (id) => {
    ...
    try {
      await api.updateKid(id, { name: newName });
      await refreshKids();
    } catch {
  ...
  const updateGrade = async (id, g) => {
    try {
      await api.updateKid(id, { grade: Number(g) });
      await refreshKids();
    } catch {}
  };
...
function ChoresManager({ kids, activeKid, setActiveKid }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (ch == null) ch = DEFAULT_CHORES.map((t) => ({ id: uid(), title: t }));
      setChores(ch);
    })();
  }, [activeKid]);
  ...
  const add = () => { if (!title.trim()) return; save([...chores, { id: uid(), title: title.trim() }]); setTitle(""); };
  ...
  return (
    <div className="sq-card" style={panel}>
      ...
      {chores.map((c) => (
        <div key={c.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 0" }}>
...
function PasswordManager() {
  ...
  const change = async () => {
    setMsg(""); setErr("");
    if (n1.length < 6) return setErr("New password must be at least 6 characters.");
    if (n1 !== n2) return setErr("New passwords don't match.");
    setBusy(true);
    try {
  ...
  return (
    <div className="sq-card" style={{ ...panel, maxWidth: 440 }}>
      <h2 className="sq-h" style={{ ...h2, marginTop: 0 }}>Change Account Password</h2>
      ...
      {err && <div style={errBox}>{err}</div>}
      {msg && <div style={{ ...errBox, background: "#eefaf0", color: "#2fa84f" }}>{msg}</div>}
      <button style={{ ...btnPrimary, opacity: busy ? 0.6 : 1 }} disabled={busy} onClick={change}>{busy ? "Updating…" : "Update password"}</button>
    </div>
  );
}
